...
import { CLOTHING_RATES, CLEANING_SERVICES } from "../data/constants";
    ...
    <div>
      ...
      <div className="rw-section" style={{ paddingTop: 0 }}>
        <div className="rw-section-head">
          ...
          <h2>Priced by weight, same as in store</h2>
          <p>Base rate per kilogram, service level (wash & iron, dry clean, express) is applied on top.</p>
        </div>
        <div className="rw-grid-4">
          {CLOTHING_RATES.map((c) => (
            <div className="rw-card" key={c.id} style={{ textAlign: "center" }}>
              <div className="rw-price" style={{ fontSize: 24 }}>{money(c.rate)}<span style={{ fontSize: 14, color: "var(--ink-soft)", fontWeight: 500 }}>/kg</span></div>
              <div style={{ marginTop: 8, fontSize: 14.5, color: "var(--ink-soft)" }}>{c.label}</div>
            </div>
          ))}
        </div>
      </div>
